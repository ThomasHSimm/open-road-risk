...
def build_traffic_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["hgv_pct_best"] = np.where(
        ...
        df["hgv_pct"],          # WebTRIS sensor-measured HGV %
        df["hgv_pct_aadf"],     # AADF count-based HGV proportion %
    )
    ...
    df["weekday_weekend_ratio"] = np.where(
        df["weekday_flow"].notna() & (df["weekday_flow"] > 0),
        df["all_flow"] / df["weekday_flow"],
        np.nan,
    )
...
def build_confidence_flags(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["has_webtris"] = df["all_flow"].notna()
