...
# Using lat/lon here since AADF has lat/lon but not BNG directly.
_bbox = cfg["study_area"]["bbox_wgs84"]
_DEFAULT_LAT_BOUNDS = (_bbox["min_lat"], _bbox["max_lat"])
_DEFAULT_LON_BOUNDS = (_bbox["min_lon"], _bbox["max_lon"])
...
def load_aadf(
    ...
    lat_bounds: tuple | None = _DEFAULT_LAT_BOUNDS,
    lon_bounds: tuple | None = _DEFAULT_LON_BOUNDS,
    ...
    if lat_bounds is not None and "latitude" in df.columns:
        before = len(df)
        df = df[df["latitude"].between(*lat_bounds)]
        logger.info(f"  Lat filter {lat_bounds}: {before:,} → {len(df):,} rows")

    if lon_bounds is not None and "longitude" in df.columns:
        before = len(df)
