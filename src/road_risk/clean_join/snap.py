...
ROAD_CLASS_SCORES = {
    1: {  # Motorway
        "preferred": {"Motorway"},
        "partial": {"A Road"},
        "penalty": {"B Road", "Classified Unnumbered", "Unclassified", "Not Classified", "Unknown"},
    },
    ...
    6: {  # Unclassified
        "preferred": {"Unclassified", "Not Classified", "Unknown", "Classified Unnumbered"},
        "partial": {"B Road"},
        "penalty": {"Motorway"},
    },
}
...
JUNCTION_SCORES = {
    0: {  # Not at junction — should NOT be on slip road or roundabout
        "preferred": {"Single Carriageway", "Dual Carriageway", "Collapsed Dual Carriageway"},
...
def _densify_links(
    ...
    logger.info(
        f"  Densified {len(links_bng):,} links → {len(xy):,} points at {interval_m}m interval"
    )
    return xy, link_ids
...
def snap_weighted(
    ...
    for i, (row_distances, row_indices) in enumerate(zip(distances, indices, strict=False)):
        ...
        seen = {}
        for dist, lid in zip(dists_raw, point_ids, strict=False):
        ...
        cand_indices = [link_id_to_idx[lid] for lid in cand_link_ids if lid in link_id_to_idx]
        if not cand_indices:
            results.append(
                {
                    "link_id": pd.NA,
...
def snap_quick(
    ...
    """
    logger.info(f"snap_quick: {len(collisions):,} collisions | cap={cap_m}m")
...
def compare_snaps(
    ...
    # Summary stats
    both_matched = comp["w_method"].isin(["weighted"]) & comp["q_method"].isin(["spatial"])
...
def main() -> None:
    ...
    print("\n=== Snap comparison ===")
    print(
        f"  Weighted matched : {(snapped_w['snap_method'] == 'weighted').sum():,} "
        f"({(snapped_w['snap_method'] == 'weighted').mean():.1%})"
    )
    print(
        f"  Quick matched    : {(snapped_q['snap_method'] == 'spatial').sum():,} "
    ...
    joined = snapped_w.merge(
        openroads[["link_id", "road_classification", "form_of_way"]], on="link_id", how="left"
    )
