...
description: "Source notes for STATS19 police-reported injury collision data, including coverage, fields, filtering, and use in collision modelling."
format:
    html:
        toc: true
        toc-depth: 3
...
3. What does the Great Britain model sample look like over 2015–2024?
...
# Configured GB bounding box check
in_bbox = (
    collisions["latitude"].between(GB_BBOX["min_lat"], GB_BBOX["max_lat"]) &
    collisions["longitude"].between(GB_BBOX["min_lon"], GB_BBOX["max_lon"])
)
out_bbox = collisions[
    collisions[["latitude","longitude"]].notna().all(axis=1) & ~in_bbox
]
print(f"Valid coords outside configured GB bbox: {len(out_bbox):,}")
...
#| fig-cap: "Collision counts and share by severity — Great Britain 2015–2024"
...
plt.suptitle("STATS19 Great Britain 2015–2024", y=1.01)
plt.tight_layout()
plt.show()
```
...
#| fig-cap: "Collision density by severity — Great Britain 2015–2024 (COVID years included)"
...
# Filter to valid coords within the configured GB bbox
valid = (
    collisions["latitude"].between(GB_BBOX["min_lat"], GB_BBOX["max_lat"]) &
    collisions["longitude"].between(GB_BBOX["min_lon"], GB_BBOX["max_lon"]) &
...
fig.suptitle("Collision density — Great Britain 2015–2024", fontsize=13, y=1.01)
plt.tight_layout()
plt.show()
```
---
