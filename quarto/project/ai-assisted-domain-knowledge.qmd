...
The prompt protocol is published as a supporting artefact: [open-road-risk-prompt-ablation-case-study.md](lit_review/open-road-risk-prompt-ablation-case-study.md).

| Run | Inputs given | Prompt condition | Output artefact | What it tested |
|---|---|---|---|---|
| A | Paper PDF + full Open Road Risk extraction prompt and project dossier | Full structured prompt | [A extraction](lit_review/A_paper-extraction-retallack-2020-traffic-volume-accident-frequency-intersections.md) | Best-case extraction under the project workflow |
| B | Paper PDF only | Generic project-aware extraction prompt | [B extraction](lit_review/B_road_safety_paper_methodological_extraction.md) | Whether a lighter project prompt still captures the useful methods |
| C | Paper reference only, no PDF | Reference-only extraction prompt | [C extraction](lit_review/C_reference_only_methodological_extraction_retallack_ostendorf_2020.md) | Whether the model refuses unsupported specificity |
| D | Paper reference + Open Road Risk context, no PDF | Conservative extraction with project context | [D extraction](lit_review/D_retallack_ostendorf_2020_conservative_extraction.md) | Whether project context creates useful focus or false confidence |
| E | Paper PDF only | Vague summary prompt | [E extraction](lit_review/E_retallack_ostendorf_2020_for_open_road_risk.md) | What happens when the task asks for usefulness before structured extraction |
| F | Paper PDF only | Structured extraction fields, but no Open Road Risk dossier | [F extraction](lit_review/F_road_safety_methodological_extraction.md) | What structure alone adds without project-specific transferability rules |
...
After the join was fixed, the honest XGBoost figure was about **0.32**. The current full-GB retrain later produced XGBoost pseudo-R² 0.387 with full-zero training, while the Poisson GLM reached 0.602 on its own downsampled in-sample surface. Those figures are not a direct horse race, but the leak diagnosis still changed how the boosted model was interpreted: the earlier 0.86 result had been flattering the more complex model.
