---
title: "Traffic Volume EDA for AADF and WebTRIS"
date-modified: 2026-07-04
description: "Exploratory analysis of full-GB traffic volume data, AADF patterns, WebTRIS observations, exposure distributions, and model inputs."
format:
  html:
    toc: true
    toc-depth: 3
...
link across full Great Britain. Directly measured AADF count points cover only a
minority of road links, especially minor and unclassified roads, so a
gradient-boosted regressor is trained on measured count sites and applied to the
full 3.94M-link network.

The resulting AADT layer is an exposure input to the collision model. It is not
just another predictor: Stage 2 turns each link-year estimate into annual
vehicle-kilometre exposure using `AADT × link length × 365`, then models
collisions relative to that exposure and the surrounding road context. AADT is
estimated where direct counts are unavailable, and the estimates are validated
against held-out count sites.
...
| Years | 2015-2024 |
| AADT estimate rows | 39,412,990 |
| Unique AADT links | 3,941,299 |
| AADT local holdout R² | 0.821 |
| AADT spatial holdout R² | 0.777 |
| AADT CV R² | 0.824 ± 0.008 |
| Risk-score rows using this exposure layer | 3,941,299 |
| Collision model XGBoost pseudo-R² | 0.360 |
| Collision model GLM pseudo-R² | 0.505 |
...
print(f"AADF count points : {aadf['count_point_id'].nunique():,} count points × {aadf['year'].nunique()} years ({aadf.shape[0]:,} rows)")
...
if HAS_GPD and openroads is not None:
    ...
    ax.set_title(
        f"AADF count-point locations ({gdf_pts['count_point_id'].nunique():,} count points)\n"
        "Red dots = AADF count points; grey = full road network (200k link sample)"
    )
    ax.set_axis_off()
    plt.tight_layout()
    plt.show()
...
the 3.94M-link network at inference and their absence would need to be imputed
...
if not val_summary.empty:
    ...
    cv_r2 = aadt_metrics.get("cv_r2_mean", 0.824) if "aadt_metrics" in globals() else 0.824
    ax.axvline(cv_r2, color="black", linestyle="--", linewidth=1, alpha=0.5,
               label=f"GroupKFold CV R²≈{cv_r2:.3f}")
...
The estimates cover every OS Open Roads link in Great Britain for each AADF
year from 2015 to 2024. They provide a consistent exposure layer for Stage 2,
including links that have no directly observed count point.
...
if not aadt.empty and openroads is not None:
    ...
    axes[0].set_title("Estimated AADT — all 3.94M links (2023, log scale)")
...
estimates close to the measured value. This is a local sanity check; the
held-out validation above is the main check on how the AADT model performs at
count sites that were not used for fitting.
...
- The estimated AADT layer feeds Stage 2 as annual vehicle-kilometre exposure
  (`AADT × link length × 365`), so the collision model ranks links relative to
  estimated traffic volume rather than treating raw collision counts alone as
  risk.
