...
### Clean GB collision retrain

A clean full GB Stage 2 collision retrain was completed on 2026-07-01 using
the refreshed GB context feature table. The run built 21,675,570 link-year rows
and scored all 2,167,557 road links. The model included GB population,
deprivation, and rural/urban context fields, and excluded the retired
English-only IMD fields.

The XGBoost Poisson model achieved pseudo-R² = 0.325, broadly consistent with
the previous model. The GLM baseline converged with pseudo-R² = 0.566, but this
value is not directly comparable with earlier GLM runs because the GB run used
a lower zero-collision sampling ratio of 1:3 for memory reasons.
...
With ~2M link-years, statistical significance is near-guaranteed; the practical
...
- XGBoost's current clean GB run has pseudo-R² `0.325`, with temporal features
  included, while the GLM sits at pseudo-R² `0.566` on its own 1:3
  zero-collision downsampled in-sample surface.
  ...
  lift. These metrics are from the current clean GB run after the counted-only Stage 1a
