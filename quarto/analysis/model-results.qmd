...
def set_gb_wgs84_extent(ax):
    """Use the configured GB frame so old regional data does not silently crop maps."""
    ax.set_xlim(GB_BBOX["min_lon"], GB_BBOX["max_lon"])
    ax.set_ylim(GB_BBOX["min_lat"], GB_BBOX["max_lat"])
...
print(f"  XGB pseudo-R²  : {XGB_CURRENT_PR2:.3f} (current clean GB run)")
...
try:
    ...
    if rows:
        ...
        print(deprivation_df.round(4).to_string(index=False))
    else:
...
print(f"XGBoost pseudo-R²     : {XGB_CURRENT_PR2:.3f} (current clean GB run)")
