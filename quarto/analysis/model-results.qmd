...
The three IMD deciles run 1 (most deprived) -> 10 (least deprived), so a
negative coefficient means **less-deprived areas have a lower predicted
collision rate**, holding exposure and road geometry fixed. The Living
Environment **Indoors** sub-domain is used deliberately — the Outdoors
sub-domain contains road-traffic-accident indicators and would leak the outcome.
See [deprivation](../data-sources/deprivation.qmd).

```{python}
imd_terms = ["imd_decile", "imd_crime_decile", "imd_living_indoor_decile"]
try:
    rows = []
    for base in imd_terms:
    ...
    if rows:
        imd_df = pd.DataFrame(rows, columns=["term", "coef", "IRR", "pvalue"])
        print(imd_df.round(4).to_string(index=False))
    else:
        print("No IMD decile terms in the fitted GLM.")
except NameError:
...
- XGBoost's current honest post-fix baseline is about pseudo-R² `0.32`
  across five seeds (`0.323` mean, with temporal features included), while the
  GLM sits at pseudo-R² `0.347` on its own in-sample downsampled surface.
  ...
  lift. These metrics are from the current run after the counted-only Stage 1a
  filter, the Stage 2 post-event provenance guard, OSM feature enrichment, IMD
  features, terrain grade, and the GLM optional-feature imputation refactor.
  See
